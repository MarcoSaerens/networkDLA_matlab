function CLU = Alg_08_07_LouvainMethod(A,mix)
...
while conv<N   % Repeat iterations until no move of a node occurs
  ...
  neigh_i = (A(i,:) > 0);   % Neighbors of i (according to A)
...
% Save first round results / Output for later rounds:
if mix
  CLU.assigns{1} = origassigns';
else
  CLU.assigns{1} = assigns';
end
...
level = level +1;
% Initialize the binary membership matrix
N = size(A_merge,1);  % N = # of nodes
newassigns = 1:N;    % Assign each node initially to its own cluster.
U = eye(N);   % Cluster membership matrix
...
%% Convergence Step
 ...
 % Check convergence:
  if fullassigns == oldassigns
    CLU.U = LMBOP_reorganize(origassigns);
    break;
  end
